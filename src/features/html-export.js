function buildExportHTML(opts) {
  ...
  function renderCurrentStateForExport(diagram) {
    ...
    getSortedNodeLayerEntries().forEach(entry => {
      ...
      const detailBtn = hasDetail && n.type !== 'boundary'
        ? `<button type="button" class="detail-btn" data-node-detail-id="${escapeHtml(String(n.id))}" aria-label="Open detail for ${escapeHtml((n.title || n.tag || 'Untitled node').replace(/\n/g, ' '))}">i</button>`
        : '';
  ...
  const rootDetailJson = serializeForScript(rootRendered?.detailData || {});
...
.detail-btn{position:absolute;top:7px;right:8px;width:14px;height:14px;border-radius:999px;background:transparent;border:1px solid var(--accent3);color:var(--accent3);font-size:8px;font-style:italic;font-family:Georgia,serif;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;opacity:0.78;transition:opacity 0.15s,background 0.15s,border-color 0.15s;}
.detail-btn:hover{opacity:1;background:var(--surface2);border-color:var(--accent2);}
...
function renderBreadcrumbs(){const ancestors=getDiagramAncestors(_activeDiagramId); _navEl.hidden=ancestors.length<=1 && _diagramBackStack.length===0 && _diagramForwardStack.length===0; _backBtn.disabled=_diagramBackStack.length===0; _forwardBtn.disabled=_diagramForwardStack.length===0; _crumbsEl.innerHTML=ancestors.map(diagram => { const active=String(diagram.id)===String(_activeDiagramId); return '<button type="button" class="export-crumb'+(active?' active':'')+'" data-diagram-id="'+escapeHtml(diagram.id)+'"'+(active?' disabled':'')+'>'+escapeHtml(diagram.title||'Untitled diagram')+'</button>'; }).join('<span class="export-crumb-sep">/</span>');}
...
function renderActiveDiagram(){const diagram=getRenderedDiagram(_activeDiagramId); if(!diagram) return false; closeDetail(); _cr.innerHTML=diagram.markup||''; _dW=diagram.width||1200; _dH=diagram.height||800; updateHeader(diagram); renderBreadcrumbs(); restoreDiagramViewport(diagram); return true;}
